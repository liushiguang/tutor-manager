...
const StuInfoTable = ()=> {
  ...
  const [editingStudent, setEditingStudent] = useState<student>(iniStudent)
  

  // 获取学生信息,并设置students数组,以显示在页面上,依赖于currentPage和studentsPerPage
  useEffect(() => {
  ...
  const handleSearch = (studentName: string) => {
    const searchedStudents = students.filter((item: student) => item.username === studentName)
    setStudents(searchedStudents)
  }

  const handleEditStudent = (cstudent: student) => {
    setEditingStudent(cstudent) //设置正在编辑的学生信息
    setIsEditing(true) // 显示编辑框
  }

  const handleDeleteStudent = async (id: number) => {
    const confirmDelete = window.confirm('确定要删除该名用户?')
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setEditingStudent({...editingStudent, [event.target.name]: event.target.value})
  }
